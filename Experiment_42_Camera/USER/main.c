...
void jpeg_data_process(void)
{
	...
	if(ov2640_mode)//只有在JPEG格式下,才需要做处理.
	{
		...
		{
			__HAL_DMA_DISABLE(&DMADMCI_Handler);	//停止当前传输		
			while(DMA2_Stream1->CR&0X01);	//等待DMA2_Stream1可配置   
			rlen=jpeg_dma_bufsize-__HAL_DMA_GET_COUNTER(&DMADMCI_Handler);//得到剩余数据长度	
		...
		{ 
			__HAL_DMA_SET_COUNTER(&DMADMCI_Handler,jpeg_dma_bufsize);//传输长度为jpeg_dma_bufsize*4字节
...
u8 ov2640_jpg_photo(u8 *pname)
{
	...
	DCMI_DMA_Init((u32)jpeg_buf0,(u32)jpeg_buf1,jpeg_dma_bufsize,DMA_MDATAALIGN_WORD,DMA_MINC_ENABLE);//DCMI DMA配置(双缓冲模式)
	...
	DCMI_DMA_Init((u32)&LCD->LCD_RAM,0,1,DMA_MDATAALIGN_HALFWORD,DMA_MINC_DISABLE);//DCMI DMA配置   
	myfree(SRAMIN,f_jpg); 
	return res;
}
...
int main(void)
{
	...
	Show_Str(30,150,200,16,"2017年5月2日",16,0);
